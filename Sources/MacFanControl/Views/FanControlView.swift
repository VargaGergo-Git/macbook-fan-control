import SwiftUI
import AppKit
import MacFanControlCore

struct FanControlView: View {
    @ObservedObject var controller: FanController
    @Environment(\.colorScheme) private var colorScheme
    @AppStorage("macFanControl.advancedMode") private var showAdvanced = false
    @State private var copiedDiagnostics = false
    @State private var fanSpinning = false

    private var dieTemperature: Double? {
        FanCurve.hottestDieCelsius(in: controller.sensors)
    }

    private var chassis: ChassisProfile {
        controller.hardwareProfile?.chassis
            ?? ChassisProfile.detect(fanCount: controller.fans.count)
    }

    private var referenceFan: Fan? {
        controller.fans.max(by: { $0.maxRPM < $1.maxRPM })
    }

    var body: some View {
        ZStack(alignment: .top) {
            AppTheme.panelBackground(for: colorScheme).ignoresSafeArea()
            AppTheme.heroGlow(for: colorScheme)
                .frame(height: 220)
                .allowsHitTesting(false)

            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    header
                    statusBanner
                    authorizationBanner

                    if controller.fans.isEmpty && controller.sensors.isEmpty && !controller.battery.isAvailable {
                        emptyState
                    } else {
                        heroMetrics
                        presetBar
                        ThermalChartView(
                            samples: controller.history,
                            maxRPM: controller.fans.map(\.maxRPM).max() ?? 6500,
                            showGPU: showAdvanced
                        )
                        if showAdvanced {
                            curvePreview
                            systemTelemetry
                        }
                        if controller.battery.isAvailable {
                            BatteryView(snapshot: controller.battery)
                        }
                        if !controller.fans.isEmpty {
                            fanSection
                        }
                        TemperatureView(
                            sensors: showAdvanced ? sortedSensors : controller.summarySensors,
                            cpuPeak: controller.cpuPeakCelsius,
                            showSensorNames: showAdvanced
                        )
                        if !showAdvanced {
                            tipBanner
                        }
                    }

                    footer
                }
                .padding(16)
                .frame(width: 400)
            }
        }
        .frame(minWidth: 400, minHeight: 420)
        .preferredColorScheme(nil) // Follow System Settings appearance
        .onAppear {
            controller.start()
            withAnimation(.linear(duration: 1.6).repeatForever(autoreverses: false)) {
                fanSpinning = true
            }
        }
        .onReceive(NotificationCenter.default.publisher(for: NSApplication.willTerminateNotification)) { _ in
            controller.releaseAllFans()
            controller.stop()
        }
    }

    private var sortedSensors: [TemperatureSensor] {
        controller.sensors.sorted { $0.celsius > $1.celsius }
    }

    private var header: some View {
        HStack(alignment: .center, spacing: 12) {
            ZStack {
                Circle()
                    .fill(AppTheme.teal.opacity(colorScheme == .dark ? 0.22 : 0.16))
                    .frame(width: 40, height: 40)
                Image(systemName: "fanblades.fill")
                    .font(.title3.weight(.semibold))
                    .foregroundStyle(AppTheme.tealDeep)
                    .rotationEffect(.degrees(fanSpinning && controller.controlMode != .appleAuto ? 360 : 0))
            }

            VStack(alignment: .leading, spacing: 2) {
                Text("MacFanControl")
                    .font(.system(.headline, design: .rounded).weight(.bold))
                    .foregroundStyle(AppTheme.ink)
                Text(chassis.summaryLabel)
                    .font(.caption2.weight(.medium))
                    .foregroundStyle(AppTheme.tealDeep.opacity(0.9))
                    .lineLimit(1)
                Text(controller.hardwareProfile?.chipDescription ?? "MacBook")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                    .lineLimit(1)
            }

            Spacer(minLength: 8)

            VStack(alignment: .trailing, spacing: 5) {
                modeChip(controller.controlMode.badgeLabel, color: modeColor)
                modeChip(controller.thermalPressure.label, color: AppTheme.pressureColor(controller.thermalPressure))
                advancedToggle
            }
        }
    }

    private var advancedToggle: some View {
        Button {
            withAnimation(.easeInOut(duration: 0.2)) {
                showAdvanced.toggle()
            }
        } label: {
            Text(showAdvanced ? "Advanced" : "Simple")
                .font(.caption2.weight(.semibold))
                .padding(.horizontal, 8)
                .padding(.vertical, 3)
                .background(
                    Capsule().fill(
                        showAdvanced
                            ? AppTheme.teal.opacity(0.22)
                            : AppTheme.mutedFill(for: colorScheme)
                    )
                )
                .foregroundStyle(showAdvanced ? AppTheme.tealDeep : .secondary)
        }
        .buttonStyle(.plain)
        .help("Toggle advanced telemetry, curve preview, and full sensor list")
    }

    private var modeColor: Color {
        switch controller.controlMode {
        case .appleAuto: return AppTheme.calm
        case .quietCurve: return AppTheme.teal
        case .balancedCurve: return AppTheme.tealDeep
        case .performanceCurve, .fixedRPM: return AppTheme.heat
        }
    }

    private func modeChip(_ title: String, color: Color) -> some View {
        Text(title)
            .font(.caption2.weight(.semibold))
            .padding(.horizontal, 8)
            .padding(.vertical, 3)
            .background(Capsule().fill(color.opacity(colorScheme == .dark ? 0.22 : 0.16)))
            .foregroundStyle(color)
    }

    private var heroMetrics: some View {
        SurfaceCard {
            HStack(alignment: .bottom, spacing: 16) {
                VStack(alignment: .leading, spacing: 4) {
                    Text("Die")
                        .font(.caption.weight(.semibold))
                        .foregroundStyle(.secondary)
                    Text(dieTemperature.map { String(format: "%.0f°", $0) } ?? "—")
                        .font(.system(size: 36, weight: .bold, design: .rounded))
                        .monospacedDigit()
                        .foregroundStyle(AppTheme.heatColor(celsius: dieTemperature ?? 40))
                        
                    if let peak = controller.cpuPeakCelsius {
                        Text(String(format: "Session peak %.0f°", peak))
                            .font(.caption2)
                            .foregroundStyle(.tertiary)
                    }
                }

                Spacer()

                VStack(alignment: .trailing, spacing: 4) {
                    Text(controller.fans.count == 1 ? "Fan" : "Fans")
                        .font(.caption.weight(.semibold))
                        .foregroundStyle(.secondary)
                    Text(fanRPMLabel)
                        .font(.system(size: 28, weight: .bold, design: .rounded))
                        .monospacedDigit()
                        .foregroundStyle(AppTheme.tealDeep)
                        
                    Text(showAdvanced ? advancedRPMCaption : "RPM")
                        .font(.caption2)
                        .foregroundStyle(.tertiary)
                        .lineLimit(1)
                }
            }
        }
        .animation(.easeInOut(duration: 0.35), value: dieTemperature ?? 0)
        .animation(.easeInOut(duration: 0.35), value: controller.fans.map(\.actualRPM))
    }

    private var fanRPMLabel: String {
        guard let rpm = controller.fans.map(\.actualRPM).max() else { return "—" }
        return Int(rpm.rounded()).formatted(.number.grouping(.automatic))
    }

    private var advancedRPMCaption: String {
        guard let fan = referenceFan, fan.maxRPM > 0 else { return "RPM" }
        let pct = (fan.actualRPM / fan.maxRPM) * 100
        return String(format: "%.0f%% of max", pct)
    }

    private var isHoldingMaxRPM: Bool {
        guard controller.controlMode == .fixedRPM, !controller.fans.isEmpty else { return false }
        return controller.fans.allSatisfy { fan in
            abs(fan.targetRPM - fan.maxRPM) <= max(80, fan.maxRPM * 0.02)
        }
    }

    private var presetBar: some View {
        SurfaceCard(padding: 10) {
            VStack(alignment: .leading, spacing: 8) {
                Text("Cooling")
                    .font(.caption.weight(.semibold))
                    .foregroundStyle(.secondary)

                HStack(spacing: 6) {
                    presetButton("Auto", selected: controller.controlMode == .appleAuto, tint: AppTheme.calm) {
                        controller.setAutoMode()
                    }
                    presetButton("Quiet", selected: controller.controlMode == .quietCurve, tint: AppTheme.teal) {
                        controller.setQuietMode()
                    }
                    presetButton("Balanced", selected: controller.controlMode == .balancedCurve, tint: AppTheme.tealDeep) {
                        controller.setBalancedMode()
                    }
                    presetButton("Perf", selected: controller.controlMode == .performanceCurve, tint: AppTheme.heat) {
                        controller.setPerformanceMode()
                    }
                    presetButton("Max", selected: isHoldingMaxRPM, tint: AppTheme.warm) {
                        controller.setMaxSpeed()
                    }
                }

                if let preset = controller.controlMode.curvePreset {
                    let ramp = chassis.ramp(for: preset)
                    Text(String(
                        format: "%@: ramp %.0f→%.0f °C · ceiling %.0f%% · %@",
                        preset.title,
                        ramp.start,
                        ramp.end,
                        chassis.rpmCeilingFraction(for: preset) * 100,
                        chassis.kind.shortLabel
                    ))
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                }
            }
        }
        .animation(.easeOut(duration: 0.25), value: controller.controlMode)
    }

    private func presetButton(
        _ title: String,
        selected: Bool,
        tint: Color,
        action: @escaping () -> Void
    ) -> some View {
        Button(action: action) {
            Text(title)
                .font(.caption.weight(.semibold))
                .frame(maxWidth: .infinity)
                .padding(.vertical, 7)
                .background(
                    RoundedRectangle(cornerRadius: 8, style: .continuous)
                        .fill(selected ? tint.opacity(colorScheme == .dark ? 0.24 : 0.18) : AppTheme.mutedFill(for: colorScheme))
                )
                .overlay(
                    RoundedRectangle(cornerRadius: 8, style: .continuous)
                        .stroke(selected ? tint.opacity(0.5) : Color.clear, lineWidth: 1)
                )
                .foregroundStyle(selected ? tint : .primary)
        }
        .buttonStyle(.plain)
        .disabled(controller.fans.isEmpty || !controller.isAuthorized)
        .opacity(controller.fans.isEmpty || !controller.isAuthorized ? 0.45 : 1)
    }

    private var curvePreview: some View {
        let preset = controller.controlMode.curvePreset ?? .balanced
        let fan = referenceFan
        let minRPM = fan?.minRPM ?? 2000
        let maxRPM = fan?.maxRPM ?? 6000
        let samples = FanCurve.curveSamples(
            minRPM: minRPM,
            maxRPM: maxRPM,
            preset: preset,
            chassis: chassis
        )
        let liveRPM: Double? = {
            guard let temperature = dieTemperature else { return nil }
            return FanCurve.targetRPM(
                temperature: temperature,
                minRPM: minRPM,
                maxRPM: maxRPM,
                preset: preset,
                chassis: chassis
            )
        }()

        return VStack(alignment: .leading, spacing: 8) {
            HStack {
                Text("Curve preview")
                    .font(.subheadline.weight(.semibold))
                Spacer()
                Text(preset.title)
                    .font(.caption2.weight(.semibold))
                    .foregroundStyle(AppTheme.tealDeep)
            }

            SurfaceCard(padding: 10) {
                CurvePreviewChart(
                    samples: samples,
                    currentTemperature: dieTemperature,
                    currentRPM: liveRPM,
                    minRPM: minRPM,
                    maxRPM: maxRPM
                )
                .frame(height: 96)

                HStack {
                    Text(String(format: "40–100 °C → %.0f–%.0f RPM", minRPM, minRPM + (maxRPM - minRPM) * chassis.rpmCeilingFraction(for: preset)))
                        .font(.caption2)
                        .foregroundStyle(.secondary)
                    Spacer()
                    if let liveRPM, let dieTemperature {
                        Text(String(format: "Now %.0f° → %.0f RPM", dieTemperature, liveRPM))
                            .font(.caption2.monospacedDigit().weight(.medium))
                            .foregroundStyle(AppTheme.tealDeep)
                    }
                }
                .padding(.top, 6)
            }
        }
    }

    private var systemTelemetry: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("System")
                .font(.subheadline.weight(.semibold))

            SurfaceCard(padding: 10) {
                VStack(spacing: 7) {
                    telemetryRow("Model", chassis.modelIdentifier)
                    telemetryRow("Chassis", chassis.kind.shortLabel)
                    telemetryRow("Fans", "\(controller.fans.count)")
                    telemetryRow("Pressure", controller.thermalPressure.label)
                    telemetryRow("Helper", controller.helperStatusSummary)
                    telemetryRow("Write path", controller.writePathSummary)
                    if let profile = controller.hardwareProfile {
                        telemetryRow("Ftst key", profile.hasFtstKey ? "Present" : "Absent")
                        telemetryRow("macOS", profile.macOSVersion)
                    }
                    telemetryRow("Sensors", "\(controller.sensors.count) keys")
                    telemetryRow("History", "\(controller.history.count) samples")
                    Text(chassis.kind.tip)
                        .font(.caption2)
                        .foregroundStyle(.secondary)
                        .frame(maxWidth: .infinity, alignment: .leading)
                        .padding(.top, 2)
                }
            }
        }
    }

    private func telemetryRow(_ title: String, _ value: String) -> some View {
        HStack {
            Text(title)
                .font(.caption)
                .foregroundStyle(.secondary)
            Spacer()
            Text(value)
                .font(.caption.monospacedDigit())
                .foregroundStyle(AppTheme.ink.opacity(0.9))
                .lineLimit(1)
        }
    }

    private var tipBanner: some View {
        Text(chassis.kind.tip)
            .font(.caption2)
            .foregroundStyle(AppTheme.tealDeep.opacity(0.95))
            .padding(.horizontal, 10)
            .padding(.vertical, 8)
            .frame(maxWidth: .infinity, alignment: .leading)
            .background(
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .fill(AppTheme.teal.opacity(colorScheme == .dark ? 0.16 : 0.10))
            )
    }

    private var statusBanner: some View {
        Text(controller.statusMessage)
            .font(.caption)
            .foregroundStyle(.secondary)
            .fixedSize(horizontal: false, vertical: true)
            .frame(maxWidth: .infinity, alignment: .leading)
    }

    @ViewBuilder
    private var authorizationBanner: some View {
        if controller.isReadOnly || controller.hardwareProfile == nil {
            EmptyView()
        } else if controller.isAuthorized {
            HStack(spacing: 6) {
                Image(systemName: "checkmark.shield.fill")
                    .foregroundStyle(AppTheme.calm)
                    .font(.caption)
                Text("Helper installed — presets and slider stay unlocked")
                    .font(.caption2.weight(.medium))
                Spacer()
            }
            .padding(.horizontal, 10)
            .padding(.vertical, 7)
            .background(
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .fill(AppTheme.calm.opacity(colorScheme == .dark ? 0.18 : 0.12))
            )
        } else {
            SurfaceCard {
                VStack(alignment: .leading, spacing: 8) {
                    Text("Allow fan control once")
                        .font(.subheadline.weight(.semibold))
                    Text("macOS asks for your administrator password to install a small helper. After that, Auto, Quiet, Balanced, Performance, Max, and the slider will not ask again — on Air and Pro alike.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                    Button("Allow fan control…") {
                        controller.authorize()
                    }
                    .buttonStyle(.borderedProminent)
                    .tint(AppTheme.tealDeep)
                    .controlSize(.small)
                }
            }
        }
    }

    private var emptyState: some View {
        SurfaceCard {
            VStack(alignment: .leading, spacing: 8) {
                ProgressView()
                    .controlSize(.small)
                Text("Looking for fans and sensors…")
                    .font(.subheadline.weight(.semibold))
                Text(controller.statusMessage)
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
    }

    private var fanSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text(controller.fans.count == 1 ? "Cooling fan" : "Fans")
                .font(.subheadline.weight(.semibold))

            ForEach(controller.fans) { fan in
                FanCard(
                    fan: fan,
                    fanCount: controller.fans.count,
                    isManual: controller.controlMode != .appleAuto,
                    isEnabled: controller.isAuthorized,
                    showAdvanced: showAdvanced
                ) { rpm in
                    controller.setFanSpeed(fanID: fan.id, rpm: rpm)
                }
            }
        }
    }

    private var footer: some View {
        HStack {
            Button(copiedDiagnostics ? "Copied diagnostics" : "Copy diagnostic info") {
                NSPasteboard.general.clearContents()
                NSPasteboard.general.setString(controller.copyDiagnostics(), forType: .string)
                copiedDiagnostics = true
                DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
                    copiedDiagnostics = false
                }
            }
            .buttonStyle(.link)
            .font(.caption)

            Spacer()

            Button("Quit") {
                controller.releaseAllFans()
                controller.stop()
                NSApplication.shared.terminate(nil)
            }
            .buttonStyle(.borderless)
            .foregroundStyle(.secondary)
            .controlSize(.small)
        }
    }
}

private struct CurvePreviewChart: View {
    let samples: [(temperature: Double, rpm: Double)]
    let currentTemperature: Double?
    let currentRPM: Double?
    let minRPM: Double
    let maxRPM: Double

    var body: some View {
        GeometryReader { geometry in
            let width = geometry.size.width
            let height = geometry.size.height
            let temps = samples.map(\.temperature)
            let minTemp = temps.min() ?? 40
            let maxTemp = temps.max() ?? 100
            let rpmLow = min(minRPM, maxRPM)
            let rpmHigh = max(max(samples.map(\.rpm).max() ?? maxRPM, maxRPM), rpmLow + 1)

            ZStack {
                Path { path in
                    guard samples.count >= 2 else { return }
                    for (index, sample) in samples.enumerated() {
                        let x = width * CGFloat((sample.temperature - minTemp) / max(maxTemp - minTemp, 1))
                        let y = height * (1 - CGFloat((sample.rpm - rpmLow) / (rpmHigh - rpmLow)))
                        if index == 0 {
                            path.move(to: CGPoint(x: x, y: y))
                        } else {
                            path.addLine(to: CGPoint(x: x, y: y))
                        }
                    }
                }
                .stroke(AppTheme.tealDeep, style: StrokeStyle(lineWidth: 2, lineCap: .round, lineJoin: .round))

                if let currentTemperature, let currentRPM {
                    let x = width * CGFloat((currentTemperature - minTemp) / max(maxTemp - minTemp, 1))
                    let y = height * (1 - CGFloat((currentRPM - rpmLow) / (rpmHigh - rpmLow)))
                    Circle()
                        .fill(AppTheme.heat)
                        .frame(width: 8, height: 8)
                        .position(x: min(max(x, 4), width - 4), y: min(max(y, 4), height - 4))
                }
            }
        }
    }
}

private struct FanCard: View {
    let fan: Fan
    let fanCount: Int
    let isManual: Bool
    let isEnabled: Bool
    let showAdvanced: Bool
    let onSpeedChange: (Double) -> Void

    @State private var sliderValue: Double
    @State private var isEditing = false
    @Environment(\.colorScheme) private var colorScheme

    init(
        fan: Fan,
        fanCount: Int,
        isManual: Bool,
        isEnabled: Bool,
        showAdvanced: Bool,
        onSpeedChange: @escaping (Double) -> Void
    ) {
        self.fan = fan
        self.fanCount = fanCount
        self.isManual = isManual
        self.isEnabled = isEnabled
        self.showAdvanced = showAdvanced
        self.onSpeedChange = onSpeedChange
        _sliderValue = State(initialValue: fan.targetRPM)
    }

    private var title: String {
        fanCount == 1 ? "Cooling fan" : "Fan \(fan.id + 1)"
    }

    private var deltaRPM: Int {
        Int((fan.actualRPM - fan.targetRPM).rounded())
    }

    private var fillFraction: Double {
        guard fan.maxRPM > fan.minRPM else { return 0 }
        return min(max((fan.actualRPM - fan.minRPM) / (fan.maxRPM - fan.minRPM), 0), 1)
    }

    var body: some View {
        SurfaceCard {
            VStack(alignment: .leading, spacing: 10) {
                HStack(alignment: .firstTextBaseline) {
                    VStack(alignment: .leading, spacing: 2) {
                        Text(title)
                            .font(.subheadline.weight(.semibold))
                        Text("Target \(formatted(sliderValue)) RPM")
                            .font(.caption2)
                            .foregroundStyle(.secondary)
                        if showAdvanced {
                            Text(String(format: "Actual %d · Δ %+d · mode %@", formatted(fan.actualRPM), deltaRPM, "\(fan.mode.rawValue)"))
                                .font(.caption2.monospacedDigit())
                                .foregroundStyle(.tertiary)
                        }
                    }
                    Spacer()
                    VStack(alignment: .trailing, spacing: 0) {
                        Text(formatted(fan.actualRPM))
                            .font(.title2.monospacedDigit().weight(.semibold))
                            .foregroundStyle(AppTheme.tealDeep)
                        Text("RPM")
                            .font(.caption2)
                            .foregroundStyle(.secondary)
                    }
                }

                if showAdvanced {
                    GeometryReader { geo in
                        ZStack(alignment: .leading) {
                            Capsule()
                                .fill(AppTheme.mutedFill(for: colorScheme))
                            Capsule()
                                .fill(AppTheme.teal.opacity(0.85))
                                .frame(width: geo.size.width * fillFraction)
                        }
                    }
                    .frame(height: 5)
                }

                Slider(
                    value: $sliderValue,
                    in: range,
                    onEditingChanged: { editing in
                        isEditing = editing
                        if !editing {
                            onSpeedChange(sliderValue)
                        }
                    }
                )
                .tint(isManual ? AppTheme.heat : AppTheme.teal)
                .disabled(!isEnabled || fan.maxRPM <= fan.minRPM)

                HStack {
                    Text("\(formatted(fan.minRPM)) min")
                    Spacer()
                    Text("\(formatted(fan.maxRPM)) max")
                }
                .font(.caption2)
                .foregroundStyle(.secondary)
            }
        }
        .onChange(of: fan.targetRPM) { newValue in
            guard !isEditing else { return }
            sliderValue = newValue
        }
        .onChange(of: fan.id) { _ in
            sliderValue = fan.targetRPM
        }
    }

    private var range: ClosedRange<Double> {
        let minRPM = min(fan.minRPM, fan.maxRPM)
        let maxRPM = max(fan.minRPM, fan.maxRPM)
        if maxRPM <= minRPM {
            return 0...1
        }
        return minRPM...maxRPM
    }

    private func formatted(_ value: Double) -> String {
        Int(value.rounded()).formatted(.number.grouping(.automatic))
    }

    private func formatted(_ value: Int) -> String {
        value.formatted(.number.grouping(.automatic))
    }
}
