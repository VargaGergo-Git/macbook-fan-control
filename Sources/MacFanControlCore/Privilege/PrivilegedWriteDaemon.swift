...
/// Root helper that listens on a unix socket. launchd owns the process so it
/// survives the administrator dialog and later slider moves.
enum PrivilegedWriteDaemon {
    static let launchdLabel = "com.macfancontrol.helper"
    static let launchdPlistPath = "/Library/LaunchDaemons/com.macfancontrol.helper.plist"
    static let installedHelperPath = "/usr/local/libexec/MacFanControlHelper"

    static func socketPath(for uid: uid_t = getuid()) -> String {
        "/tmp/macfancontrol-\(uid).sock"
    }

    static func logPath(for uid: uid_t = getuid()) -> String {
        "/tmp/macfancontrol-\(uid).log"
    }
    ...
    static func ping(socketPath: String, timeoutSeconds: TimeInterval = 0.6) -> Bool {
